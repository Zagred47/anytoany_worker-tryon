...
def load_models(device=device, torch_dtype=torch_dtype,group_offloading=False, base_path="downloaded_models"):
    ...
    text_encoder = CLIPTextModel.from_pretrained(
        os.path.join(base_path, "text_encoder", "text_encoder"),
        torch_dtype=torch_dtype
    ).to(device)

    text_encoder_2 = T5EncoderModel.from_pretrained(
        os.path.join(base_path, "text_encoder_2", "text_encoder_2"),
        torch_dtype=torch_dtype
    ).to(device)

    transformer = FluxTransformer2DModel.from_pretrained(
        os.path.join(base_path, "transformer", "transformer"),
        torch_dtype=torch_dtype
    ).to(device)

    vae = AutoencoderKL.from_pretrained(
        os.path.join(base_path, "vae", "vae"),
        torch_dtype=torch_dtype
    ).to(device)
